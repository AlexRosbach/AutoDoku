...
def _row_to_device(row: dict[str, str]) -> Device:
    ...
    return Device(
        ...
        snmp_switch=row.get("SNMP-Switch", "").strip(),
        snmp_port=row.get("SNMP-Port", "").strip(),
        tls_certificates=row.get("TLS-Zertifikate", "").strip(),
        mdns_summary=row.get("mDNS", "").strip(),
        upnp_summary=row.get("UPnP/SSDP", "").strip(),
        passive_discovery=row.get("Passive Discovery", "").strip(),
        identity_confidence=row.get("Identity Confidence", "").strip(),
